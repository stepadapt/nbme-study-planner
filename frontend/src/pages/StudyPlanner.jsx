...
export default function StudyPlanner({ onShowTerms }) {
  ...
  // Runs once after data loads. If the stored plan was generated by an older
  // engine version, regenerate it transparently using the current algorithm.
  // Preserves all user data (scores, profile, sticking points). Fails silently.
  useEffect(() => {
    ...
    // Don't regenerate past-exam plans
    if (profile.examDate && new Date(profile.examDate) < new Date()) return;

    // Derive inputs from current assessments state (same pattern as regeneratePlanFromAssessments)
    ...
    try {
      ...
      api.plans.save({
        ...
        setLatestPlanMeta({ id: result.id, createdAt: result.createdAt || latestPlanMeta.createdAt, engineVersion: PLAN_ENGINE_VERSION });
