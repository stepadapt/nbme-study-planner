...
import { generatePlan, generateFirstTimerPlan, getTopSubTopics, getQbankFilterTip, getPerformanceLevel, assignBlockTimes, findTodayInPlan, calcPlanProgress, formatDuration, roundToQuarterHour, PLAN_ENGINE_VERSION, getChangesSince } from '../planEngine.js';
      ...
      {sequence.map((step, si) => {
        ...
        return (
          <div key={si} style={{ paddingBottom: si < sequence.length - 1 ? (compact ? 7 : 9) : 0, marginBottom: si < sequence.length - 1 ? (compact ? 7 : 9) : 0, borderBottom: si < sequence.length - 1 ? '1px solid rgba(0,0,0,0.05)' : 'none' }}>
            ...
            {/* Static channel buttons — all four shown for every WATCH step */}
            {act === 'WATCH' && (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, paddingLeft: compact ? 18 : 20, marginTop: 0 }}>
                {staticChannels.map((ch, li) => (
                  <a key={li} href={ch.url} target="_blank" rel="noopener noreferrer"
...
function getICSEventDescription(block) {
  switch (block.type) {
    case 'anki': return block.tasks?.map(t => t.activity).filter(Boolean).join(' ') || block.label || 'Morning retention block.';
...
export default function StudyPlanner({ onShowTerms }) {
  ...
  // ── Reschedule upcoming exam state ────────────────────────────────
  // reschedulingExam = { test, day, label, currentDate } when modal is open
  const [reschedulingExam, setReschedulingExam] = useState(null);
  ...
  //
  // A) RECOVERY — plan was incorrectly reset to today by the old regeneration
  //    bug (version bump caused a POST inserting a new row instead of a PUT
  //    updating the existing one). Detected by: plan.createdAt is today BUT an
  //    active assessment exists from a prior date (proving the student was already
  //    studying). Recovers by regenerating from the original start date (earliest
  //    active assessment date) and restoring that date in the DB.
  //
  // B) VERSION UPGRADE — plan engine version bumped. Regenerates plan from the
  //    ORIGINAL plan start date (latestPlanMeta.createdAt), then PUTs the updated
  //    plan_data in-place. created_at is preserved → calendarDay offsets stay
  //    correct → student stays on the right day.
  useEffect(() => {
    if (!dataLoaded || !plan || !latestPlanMeta) return;

    // Don't touch past-exam plans
    if (profile.examDate && new Date(profile.examDate) < new Date()) return;

    // ── Shared input derivation ─────────────────────────────────────────
    const sorted = [...assessments].sort((a, b) =>
      new Date(a.takenAt || a.created_at) - new Date(b.takenAt || b.created_at)
    );
    ...
    // ── Case B: Version upgrade — regenerate from original start date ──
    const planVersion = latestPlanMeta.engineVersion || 0;
    if (planVersion >= PLAN_ENGINE_VERSION) return; // already current — nothing to do

    const oldVersion = planVersion;
    // Use the ORIGINAL plan start date so calendarDay offsets are preserved.
    // Passing planStartDate prevents the plan from resetting the student to Day 1.
    ...
    try {
      ...
      api.plans.update(latestPlanMeta.id, {
        ...
        if (changes.length > 0) setPlanUpdateBanner(changes);
      }).catch(err => {
        console.error('[plan-regen] Save failed — old plan preserved:', err);
      });
    } catch (err) {
      console.error('[plan-regen] Generation failed — old plan preserved:', err);
    }
  }, [dataLoaded]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const regeneratePlanFromAssessments = (updatedAssessments, profileOverride = null) => {
    ...
    // Prune stale reschedule overrides: any whose testId is now taken, date is in
    // the past, or refers to Free 120 (which is never overridable). Use profileOverride
    // if supplied (so the reschedule save flow uses its own freshly-merged overrides).
    const sourceProfile = profileOverride || profile;
    const todayFlat = new Date(); todayFlat.setHours(0, 0, 0, 0);
    const takenIds = new Set(derivedTaken.map(t => t.id));
    const rawOverrides = sourceProfile.scheduledAssessmentOverrides || {};
    const prunedOverrides = {};
    let overridesChanged = false;
    for (const [testId, dateStr] of Object.entries(rawOverrides)) {
      if (testId === 'free120new' || testId === 'free120old') { overridesChanged = true; continue; }
      if (takenIds.has(testId)) { overridesChanged = true; continue; }
      const d = dateStr ? new Date(dateStr) : null;
      if (!d || isNaN(d.getTime())) { overridesChanged = true; continue; }
      d.setHours(0, 0, 0, 0);
  ...
  // Open the reschedule modal for an upcoming exam (from assessmentSchedule).
  // Item shape: { day, test, label, ... } as returned by scheduleAssessments.
  // Free 120 is locked at T-2 and cannot be moved — caller is expected to gate,
  // but we also guard defensively here.
  const openRescheduleExam = (item) => {
    ...
    if (!startDate) return;
    const cur = new Date(startDate); cur.setDate(cur.getDate() + item.day - 1);
    const ymd = `${cur.getFullYear()}-${String(cur.getMonth() + 1).padStart(2, '0')}-${String(cur.getDate()).padStart(2, '0')}`;
    setReschedulingExam({ test: item.test, day: item.day, label: item.label, currentDate: ymd });
    setRescheduleDate(ymd);
  };

  const saveRescheduleExam = async () => {
    if (!reschedulingExam) return;
    const testId = reschedulingExam.test.id;
    if (testId === 'free120new' || testId === 'free120old') return;
    if (!rescheduleDate) return;
    setRescheduleSaving(true);
    try {
      const newOverrides = { ...(profile.scheduledAssessmentOverrides || {}), [testId]: rescheduleDate };
      const newProfile = { ...profile, scheduledAssessmentOverrides: newOverrides };
      setProfile(newProfile);
      await api.profile.save(newProfile);
      regeneratePlanFromAssessments(assessments, newProfile);
      setReschedulingExam(null);
      setAssessmentActionMsg(`${reschedulingExam.test.name} rescheduled — your plan has been recalculated around the new date.`);
      setTimeout(() => setAssessmentActionMsg(''), 5000);
    } catch (err) {
      setAssessmentActionMsg('Failed to reschedule: ' + (err.message || 'Please try again.'));
    } finally {
      setRescheduleSaving(false);
    }
  };

  // Reset an overridden exam back to engine-chosen placement.
  const resetRescheduleExam = async () => {
    if (!reschedulingExam) return;
    const testId = reschedulingExam.test.id;
    setRescheduleSaving(true);
    try {
      ...
      regeneratePlanFromAssessments(assessments, newProfile);
      setReschedulingExam(null);
      setAssessmentActionMsg(`${reschedulingExam.test.name} reset to engine-chosen date.`);
      setTimeout(() => setAssessmentActionMsg(''), 5000);
    } catch (err) {
              ...
              <>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10, flexWrap: 'wrap' }}>
                  ...
                  {day.dayType === 'nbme' && (() => {
                    ...
                    if (item.test.id === 'free120new' || item.test.id === 'free120old') {
